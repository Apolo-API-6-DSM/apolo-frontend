...
import CardSimples from '@/components/chamados/cardSimples/CardSimples';
...
const ListaChamado = () => {
  ...
  const nomeArquivo = searchParams.get('nomeArquivo');
  ...
  const handleFilter = async (filters: FilterState) => {
    ...
    try {
      ...
      if (filters.sentimento_cliente) {
        filteredData = filteredData.filter(chamado =>
          chamado?.sentimento_cliente?.toLowerCase().includes(filters.sentimento_cliente.toLowerCase()) // Adicionando verificação de undefined
        );
      }
      if (filters.tipo_importacao) {
        filteredData = filteredData.filter(chamado =>
          chamado?.tipo_importacao?.toLowerCase().includes(filters.tipo_importacao.toLowerCase()) // Adicionando verificação de undefined
        );
      }
      if (filters.dataInicio || filters.dataFim) {
        filteredData = filteredData.filter(chamado => {
          let matchesFilter = true;
          if (filters.dataInicio && chamado?.data_abertura) { // Adicionando verificação de undefined
          ...
          if (filters.dataFim && chamado?.ultima_atualizacao) { // Adicionando verificação de undefined
      ...
      if (filters.responsavel) {
        filteredData = filteredData.filter(chamado =>
          chamado?.responsavel?.toLowerCase().includes(filters.responsavel.toLowerCase()) // Adicionando verificação de undefined
        );
      }
  ...
  const handleSearch = () => {
    ...
    const searchedCalls = allCalls.filter((chamado) =>
      (chamado?.responsavel?.toLowerCase().includes(searchLower)) || // Adicionando verificação de undefined
      (chamado?.sentimento_cliente?.toLowerCase().includes(searchLower)) || // Adicionando verificação de undefined
      (chamado?.tipo_importacao?.toLowerCase().includes(searchLower)) || // Adicionando verificação de undefined
      (chamado?.status?.toLowerCase().includes(searchLower)) // Adicionando verificação de undefined
    );
      ...
      <div className="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] lg:p-6">
        ...
        {isLoading ? (
          ...
          <div className="rounded-lg bg-red-100 p-4 text-red-700 dark:bg-red-900/20 dark:text-red-300">
            <p>{error}</p>
            <button onClick={fetchChamados} className="mt-2 inline-flex items-center justify-center gap-2 rounded-full border border-transparent bg-[#00163B] px-4 py-2 text-sm font-medium text-white shadow-theme-xs hover:bg-[#001e4f] dark:bg-blue-700 dark:hover:bg-blue-800">
              Tentar novamente
            </button>
          </div>
        ) : calls.length > 0 ? (
